...
// Creating a tree from pre order traversal
...
void BSTFromPre(int pre[],int n)
{
    ...
    while(i<n)
    {
        ...
        {
            if(pre[i]>t->data && (pre[i]<s.stackTop() || true))
            {
                p = new Node;
                p->data = pre[i++];
                p->lchild = p->rchild = NULL;
...
// outside preorder (outside the class
